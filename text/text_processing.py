...
class TextProcessor(object):
    def __init__(self, symbol_set, cleaner_names, handle_sil=False, skip_spaces=False):
        ...
        self.padding_idx = self.symbols.index('_') 
    ...
    def clean_text(self, text, lang):
        cleaner = getattr(cleaners, self.cleaner_names[int(lang)])
        if not cleaner:
            raise Exception('Unknown cleaner: %s' % self.cleaner_names[int(lang)])
        text = cleaner(text)
        return text

    def encode_text(self, text, lang):
